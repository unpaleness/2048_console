...
// Нет смысла отдельно объявлять сигнатуру функций, а затем сразу писать сами функции.
// Если функция полностью описана до main, то она всё-равно будет inline.
short power2(short);
...
int main(void)
{
    ...
    // Операция выделения памяти достаточно дорогая, и выделять её в цикле,
    // вот так явно - не эффективно. Лучше выделить сразу большой одномерный
    // кусок памяти, размером W*W. Для этого случая нужна будет функция, которая
    // будет принимать указатель на одномерный массив, его размер и координаты (x, y).
    // Например:
    //
    //     short get_value(short *a, short W, short x, short y);
    //
    // Вызов этой функции можно замакросить через #define, скажем указав в дефайне
    // дефолтное название переменной, в которой лежит размен (W). Но в таком случае
    // придётся везде предоставлять размер массива в переменной с одним и тем же
    // названием. Например (покахивает лиспом):
    //
    //     #define i(a, x, y) \
    //       (get_value((a), W, (x), (y)))
    //
    // Но конечно дефайны - это треш, и постулаты с++ гласят, что надо использовать
    // классы и месить ими.
    //
    short **a = new short *[W];
    for(short i = 0; i < W; i++)
